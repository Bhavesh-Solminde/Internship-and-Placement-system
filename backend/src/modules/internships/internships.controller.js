...
// ─── Browse internships (public — hides expired) ────────────────────
export const listInternships = asyncHandler(async (req, res) => {
  ...
  const conditions = ["(i.deadline IS NULL OR i.deadline > NOW())"];
  const params = [];
  let idx = 1;

  if (company) { conditions.push(`c.name ILIKE $${idx++}`); params.push(`%${company}%`); }
  if (location) { conditions.push(`c.location ILIKE $${idx++}`); params.push(`%${location}%`); }
  if (status) { conditions.push(`i.status = $${idx++}`); params.push(status); }
  else { conditions.push(`i.status = 'open'`); }
  ...
  const { rows } = await pool.query(
    `SELECT i.*, c.name AS company_name, c.industry, c.location AS company_location, c.company_id
     FROM internships i
...
export const getInternship = asyncHandler(async (req, res) => {
  const { rows } = await pool.query(
    `SELECT i.*, c.name AS company_name, c.industry, c.location AS company_location, c.website, c.company_id
     FROM internships i
     JOIN companies c ON i.company_id = c.company_id
...
export const createInternship = asyncHandler(async (req, res) => {
  const { title, stipend, duration, description, required_experience_years, deadline } = req.body;
  if (!title) throw new ApiError(400, "Title is required");

  const { rows } = await pool.query(
    `INSERT INTO internships (title, stipend, duration, description, company_id, required_experience_years, deadline)
     VALUES ($1, $2, $3, $4, $5, $6, $7)
     RETURNING *`,
    [title, stipend || 0, duration || null, description || null, req.user.id, required_experience_years || 0, deadline || null]
  );
...
export const updateInternship = asyncHandler(async (req, res) => {
  const { title, stipend, duration, description, status, required_experience_years, deadline } = req.body;
  const { rows } = await pool.query(
    `UPDATE internships
     SET title = COALESCE($1, title), stipend = COALESCE($2, stipend),
         duration = COALESCE($3, duration), description = COALESCE($4, description),
         status = COALESCE($5, status), required_experience_years = COALESCE($6, required_experience_years),
         deadline = COALESCE($7, deadline)
     WHERE internship_id = $8 AND company_id = $9 RETURNING *`,
    [title||null, stipend||null, duration||null, description||null, status||null,
     required_experience_years!=null?required_experience_years:null, deadline||null, req.params.id, req.user.id]
  );
...
export const applyToInternship = asyncHandler(async (req, res) => {
  ...
  const internship = await pool.query(
    `SELECT status, deadline FROM internships WHERE internship_id = $1`, [id]
  );
