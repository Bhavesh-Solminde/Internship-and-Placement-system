...
// ─── Browse jobs (public — hides expired) ────────────────────────────
export const listJobs = asyncHandler(async (req, res) => {
  const { company, location, status, search, page = 1, limit = 20 } = req.query;
  const offset = (page - 1) * limit;
  const conditions = ["(j.deadline IS NULL OR j.deadline > NOW())"];
  const params = [];
  let idx = 1;
  ...
  else { conditions.push(`j.status = 'open'`); }
  ...
  const { rows } = await pool.query(
    `SELECT j.*, c.name AS company_name, c.industry, c.location AS company_location, c.company_id
     FROM jobs j
...
export const getJob = asyncHandler(async (req, res) => {
  const { rows } = await pool.query(
    `SELECT j.*, c.name AS company_name, c.industry, c.location AS company_location, c.website, c.company_id
     FROM jobs j
     JOIN companies c ON j.company_id = c.company_id
     WHERE j.job_id = $1`,
    [req.params.id]
...
export const createJob = asyncHandler(async (req, res) => {
  const { job_title, salary, location, description, required_experience_years, deadline } = req.body;
  if (!job_title) throw new ApiError(400, "Job title is required");

  const { rows } = await pool.query(
    `INSERT INTO jobs (job_title, salary, location, description, company_id, required_experience_years, deadline)
     VALUES ($1, $2, $3, $4, $5, $6, $7)
     RETURNING *`,
    [job_title, salary || null, location || null, description || null, req.user.id, required_experience_years || 0, deadline || null]
  );
...
export const updateJob = asyncHandler(async (req, res) => {
  const { job_title, salary, location, description, status, required_experience_years, deadline } = req.body;
  const { rows } = await pool.query(
    `UPDATE jobs
     SET job_title = COALESCE($1, job_title), salary = COALESCE($2, salary),
    ...
    [job_title||null, salary||null, location||null, description||null, status||null,
...
export const applyToJob = asyncHandler(async (req, res) => {
  ...
  const job = await pool.query(
    `SELECT status, deadline FROM jobs WHERE job_id = $1`, [id]
  );
